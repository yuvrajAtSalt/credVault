...
export const exportEnvironment = async (projectId: string, envId: string, format: string, currentUser: any) => {
    const env = await EnvironmentModel.findOne({ _id: envId, projectId }).lean();
    if (!env) throw { statusCode: 404, message: 'ENVIRONMENT NOT FOUND' };
    ...
    await writeAuditLog({
        ...
        organisationId: String(currentUser.organisationId),
        meta: { format, envName: env.name, keyCount: decoded.length, projectId },
    });
